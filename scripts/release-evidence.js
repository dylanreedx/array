...
function validateLocked(root) {
  ...
  if (errors.length) throw new Error(errors.join("\n")); loaded.manifest.validated_snapshot_sha256 = sha256Text(JSON.stringify({ ...loaded.manifest, validated_at: null, validated_snapshot_sha256: null })); loaded.manifest.validated_at = new Date().toISOString(); atomicJSON(loaded.file, loaded.manifest, "manifest"); console.log(`validated ${loaded.manifest.reports.length} report(s), ${loaded.manifest.capture_manifests.length} capture manifest(s), ${loaded.manifest.canonical_artifacts.length} canonical artifact(s)`);
}
