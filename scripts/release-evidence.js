...
const REQUIRED_PERF_ROLES = [...PERF_ROLES].sort();
...
function invalidateValidation(manifest) { manifest.validated_at = null; manifest.validated_snapshot_sha256 = null; }
function validationSnapshotHash(manifest) { return sha256Text(JSON.stringify({ ...manifest, validated_at: null, validated_snapshot_sha256: null })); }
...
function loadManifest(root) {
  ...
  const hasTime = manifest.validated_at !== null, hasSnapshot = manifest.validated_snapshot_sha256 !== null; if (hasTime !== hasSnapshot || (hasTime && (!Number.isFinite(Date.parse(manifest.validated_at)) || !SHA256.test(manifest.validated_snapshot_sha256 || "") || manifest.validated_snapshot_sha256 !== validationSnapshotHash(manifest)))) throw new Error("manifest validation stamp is incoherent or stale");
...
function init(options) {
  ...
  atomicJSON(file, { schema_version: 1, run_id: options["run-id"], root: root.root, base_sha: options["base-sha"], expected_inventory: null, reports: [], capture_manifests: [], canonical_artifacts: [], validated_at: null, validated_snapshot_sha256: null }, "manifest");
}
function validateInventory(value, baseSha) {
  if (!value || typeof value !== "object" || !SHA40.test(value.candidate_sha || "") || !Array.isArray(value.workstreams) || !value.workstreams.length || value.global_performance_identity !== true) throw new Error("expected inventory schema is invalid");
  ...
  for (const stream of value.workstreams) {
    ...
    for (const role of stream.roles) { const key = `${stream.workstream}\0${role?.role}`; if (!role || !ROLES.has(role.role) || identities.has(key) || !Array.isArray(role.visual_states) || !Array.isArray(role.performance_cases) || !Array.isArray(role.capture_manifests) || (role.performance_cases.length === 0 && role.allow_zero_performance !== true)) throw new Error("expected role inventory schema is invalid"); identities.add(key); validateExpectedVisuals(role.visual_states); validateExpectedPerformance(role.performance_cases); for (const capture of role.capture_manifests) if (!capture || !["qacapture", "external"].includes(capture.kind) || typeof capture.flow !== "string" || !capture.flow) throw new Error("capture inventory schema is invalid"); }
  }
}
function inventory(options) {
  ...
  withLock(root.root, () => { const loaded = loadManifest(root); validateInventory(value, loaded.manifest.base_sha); const entry = { path: file, sha256: sha256File(file), value }; if (loaded.manifest.expected_inventory) { if (loaded.manifest.expected_inventory.sha256 !== entry.sha256) throw new Error("expected inventory is immutable once locked"); invalidateValidation(loaded.manifest); atomicJSON(loaded.file, loaded.manifest, "manifest"); return; } if (loaded.manifest.reports.length || loaded.manifest.capture_manifests.length || loaded.manifest.canonical_artifacts.length) throw new Error("expected inventory must be locked before ingestion"); loaded.manifest.expected_inventory = entry; loaded.manifest.i3_sha = value.candidate_sha; invalidateValidation(loaded.manifest); atomicJSON(loaded.file, loaded.manifest, "manifest"); });
}
function ingest(options) {
  ...
  withLock(root.root, () => { const loaded = loadManifest(root); if (!loaded.manifest.expected_inventory) throw new Error("expected inventory must be locked before ingestion"); const entry = { path: reportPath, sha256: sha256File(reportPath), workstream: report.workstream, role: report.role, base_sha: report.base_sha, candidate_sha: report.candidate_sha };
    ...
    for (const capturePathArg of options["capture-manifest"] || []) { const capturePath = contained(root, capturePathArg, "capture manifest"); const captureValue = readJSON(capturePath, "capture manifest"); validateCaptureManifest(captureValue, root, loaded.manifest.expected_inventory.value.candidate_sha); const capture = { path: capturePath, sha256: sha256File(capturePath), kind: captureValue.kind, flow: captureValue.flow, workstream: report.workstream, role: report.role }; const prior = loaded.manifest.capture_manifests.find(item => item.path === capture.path), identity = loaded.manifest.capture_manifests.find(item => item.kind === capture.kind && item.flow === capture.flow && item.workstream === capture.workstream && item.role === capture.role); if (prior && prior.sha256 !== capture.sha256) throw new Error("an ingested capture manifest changed in place"); if (identity && identity.path !== capture.path) throw new Error("duplicate capture identity at alternate path"); if (!prior) loaded.manifest.capture_manifests.push(capture); }
    loaded.manifest.reports.sort((a, b) => a.path.localeCompare(b.path)); loaded.manifest.capture_manifests.sort((a, b) => a.path.localeCompare(b.path)); invalidateValidation(loaded.manifest); atomicJSON(loaded.file, loaded.manifest, "manifest"); });
}
...
function validateExpectedPerformance(cases) { const keys = new Set(); for (const [i, item] of cases.entries()) { object(item, `performance_cases[${i}]`); string(item.case_id, `performance_cases[${i}].case_id`); integer(item.repetitions, `performance_cases[${i}].repetitions`, 5); if (keys.has(item.case_id) || !Array.isArray(item.required_roles) || JSON.stringify([...item.required_roles].sort()) !== JSON.stringify(REQUIRED_PERF_ROLES) || item.configuration !== "release" || typeof item.seed !== "string" || !item.seed.trim()) throw new Error("performance case must declare the complete release evidence contract"); keys.add(item.case_id); } }
function validateCaptureManifest(value, root, candidateSha) {
  ...
  const identities = new Set(); for (const [i, artifact] of value.artifacts.entries()) { const label = `capture artifact[${i}]`; object(artifact, label); string(artifact.case_id, `${label}.case_id`); integer(artifact.iteration, `${label}.iteration`, 1); string(artifact.visual_state_id, `${label}.visual_state_id`); if (!["actual", "failure"].includes(artifact.role) || !["aqua", "dark"].includes(artifact.appearance)) throw new Error("capture artifact role/appearance is invalid"); const identity = `${artifact.case_id}\0${artifact.iteration}\0${artifact.visual_state_id}\0${artifact.role}`; if (identities.has(identity)) throw new Error("duplicate capture artifact identity"); identities.add(identity); string(artifact.readiness, `${label}.readiness`); if (!["PASS", "FAIL", "NEEDS_JUDGMENT"].includes(artifact.inspection)) throw new Error("capture artifact inspection is invalid"); if (value.status === "PASS" && (artifact.inspection !== "PASS" || artifact.role === "failure")) throw new Error("PASS capture manifest contains non-PASS artifact"); if (value.status === "DISPLAY_DEFERRED" && (artifact.role !== "failure" || artifact.failure_reason !== "capture_unavailable" || artifact.inspection !== "FAIL")) throw new Error("DISPLAY_DEFERRED capture requires typed capture_unavailable failures"); if (artifact.role === "failure") string(artifact.failure_reason, `${label}.failure_reason`); validateVisualGeometry(artifact, label, candidateSha); const image = verifyFile(root, artifact.path, `${label}.path`, artifact.sha256, true); if (artifact.actual_path !== artifact.path || artifact.actual_sha !== image.sha256) throw new Error(`${label} actual image provenance mismatch`); verifyFile(root, artifact.semantic_artifact, `${label}.semantic_artifact`, artifact.semantic_sha256); verifyFile(root, artifact.command_log, `${label}.command_log`, artifact.command_log_sha256); validateVisualComparison(artifact, label, root); }
  if (value.status === "FAIL" && !value.artifacts.some(item => item.inspection === "FAIL" && item.role === "failure")) throw new Error("FAIL capture manifest lacks failure artifact"); if (value.status === "NEEDS_JUDGMENT" && !value.artifacts.some(item => item.inspection === "NEEDS_JUDGMENT")) throw new Error("NEEDS_JUDGMENT capture manifest lacks matching artifact");
}
function validateReportShape(report, root, deep = true) {
  ...
  if (!["clean", "dirty-with-explanation"].includes(report.candidate_dirty_status) || !["PROMOTE", "DO_NOT_PROMOTE"].includes(report.promotion_recommendation) || !path.isAbsolute(report.worktree)) throw new Error("report status/promotion/worktree schema is invalid");
  ...
  for (const field of ["owned_files_changed", "unexpected_files_changed"]) for (const [i, value] of report[field].entries()) { string(value, `${field}[${i}]`); if (!path.isAbsolute(value) || !fs.existsSync(value)) throw new Error(`${field}[${i}] must be an existing absolute path`); }
  for (const field of ["known_red_observed", "unexpected_failures", "risks", "gaps_or_skips"]) for (const [i, value] of report[field].entries()) string(value, `${field}[${i}]`);
  for (const [i, value] of report.semantic_artifacts.entries()) string(value, `semantic_artifacts[${i}]`);
  ...
  for (const [i, item] of report.review_findings.entries()) { object(item, `review_findings[${i}]`); if (!["P0", "P1", "P2", "P3"].includes(item.severity) || typeof item.resolved !== "boolean") throw new Error(`review_findings[${i}] schema is invalid`); string(item.file, `review_findings[${i}].file`); if (!path.isAbsolute(item.file)) throw new Error(`review_findings[${i}].file must be absolute`); integer(item.line, `review_findings[${i}].line`, 1); string(item.finding, `review_findings[${i}].finding`); if (!item.resolved) string(item.reproducer, `review_findings[${i}].reproducer`); }
  const unresolvedBlocking = report.review_findings.filter(item => !item.resolved && ["P0", "P1", "P2"].includes(item.severity)); if (report.promotion_recommendation === "PROMOTE" && (report.status !== "PASS" || unresolvedBlocking.length)) throw new Error("PROMOTE requires PASS with no unresolved P0/P1/P2 findings"); if (report.status !== "PASS" && report.promotion_recommendation !== "DO_NOT_PROMOTE") throw new Error(`${report.status} must recommend DO_NOT_PROMOTE`);
...
function validateVisuals(report, root) {
  ...
  for (const [index, image] of report.screenshots.entries()) {
    ...
    validateVisualGeometry(image, label, report.candidate_sha);
    ...
    if (image.role === "actual" && (!image.actual_path || !image.actual_sha)) throw new Error(`${label} actual provenance is incomplete`); if (image.actual_path) verifyFile(root, image.actual_path, `${label}.actual_path`, image.actual_sha, true);
    if (image.role === "actual") cover(key, "actual");
    if (image.role === "contact_sheet") { if (!Array.isArray(image.contact_sheet_members) || !image.contact_sheet_members.length) throw new Error(`${label} contact sheet must have members`); for (const [memberIndex, member] of image.contact_sheet_members.entries()) { const ml = `${label}.contact_sheet_members[${memberIndex}]`; object(member, ml); string(member.case_id, `${ml}.case_id`); integer(member.iteration, `${ml}.iteration`, 1); string(member.visual_state_id, `${ml}.visual_state_id`); if (member.role !== "actual" || !["aqua", "dark"].includes(member.appearance)) throw new Error(`${ml} role/appearance is invalid`); validateVisualGeometry(member, ml, report.candidate_sha); string(member.readiness, `${ml}.readiness`); if (!["PASS", "FAIL", "NEEDS_JUDGMENT"].includes(member.inspection)) throw new Error(`${ml}.inspection is invalid`); verifyFile(root, member.actual_path, `${ml}.actual_path`, member.actual_sha, true); verifyFile(root, member.semantic_artifact, `${ml}.semantic_artifact`, member.semantic_sha256); verifyFile(root, member.command_log, `${ml}.command_log`); validateVisualComparison(member, ml, root); const memberKey = `${member.visual_state_id}\0${member.appearance}`; if (!expected.has(memberKey)) throw new Error(`${ml} is outside inventory`); cover(memberKey, "contact_sheet"); } }
    validateVisualComparison(image, label, root);
  }
...
function validateVisualGeometry(item, label, candidateSha) { if (item.commit !== candidateSha || !Array.isArray(item.point_size) || item.point_size.length !== 2 || !item.point_size.every(value => Number.isFinite(value) && value > 0) || !Array.isArray(item.pixel_size) || item.pixel_size.length !== 2 || !item.pixel_size.every(value => Number.isFinite(value) && value > 0) || !Number.isFinite(item.backing_scale) || item.backing_scale <= 0 || !Number.isFinite(item.canvas_zoom) || item.canvas_zoom <= 0 || !Number.isFinite(item.tile_zoom) || item.tile_zoom <= 0 || typeof item.primary !== "boolean" || item.pixel_size.some((value, index) => value !== item.point_size[index] * item.backing_scale)) throw new Error(`${label} geometry/commit/primary provenance is invalid`); }
function validateVisualComparison(item, label, root) {
  const required = ["diff_metric", "diff_status", "baseline_path", "baseline_sha", "baseline_commit", "diff_path", "diff_sha", "mask_path", "mask_sha", "mask_rationale"]; for (const key of required) if (!Object.hasOwn(item, key)) throw new Error(`${label}.${key} is required`);
  if (!Number.isFinite(item.diff_metric) || item.diff_metric < 0 || !["zero", "nonzero", "failed", "candidate_only", "not_applicable"].includes(item.diff_status)) throw new Error(`${label} diff metric/status is invalid`);
  const allNull = keys => keys.every(key => item[key] === null), baselineKeys = ["baseline_path", "baseline_sha", "baseline_commit"], diffKeys = ["diff_path", "diff_sha"];
  if (["zero", "nonzero", "failed"].includes(item.diff_status)) { verifyFile(root, item.baseline_path, `${label}.baseline_path`, item.baseline_sha, true); if (!SHA40.test(item.baseline_commit || "")) throw new Error(`${label}.baseline_commit is invalid`); } else if (!allNull(baselineKeys)) throw new Error(`${label} baseline fields must be null for ${item.diff_status}`);
  if (["nonzero", "failed"].includes(item.diff_status)) verifyFile(root, item.diff_path, `${label}.diff_path`, item.diff_sha, true); else if (!allNull(diffKeys)) throw new Error(`${label} diff fields must be null for ${item.diff_status}`);
  if (item.diff_status === "zero" && item.diff_metric !== 0) throw new Error(`${label} zero diff requires metric 0`); if (item.diff_status === "candidate_only" && item.inspection !== "NEEDS_JUDGMENT") throw new Error(`${label} candidate-only requires NEEDS_JUDGMENT`); if (item.diff_status === "not_applicable" && item.diff_metric !== 0) throw new Error(`${label} not_applicable requires metric 0`);
  const maskKeys = ["mask_path", "mask_sha", "mask_rationale"]; if (!allNull(maskKeys)) { if (maskKeys.some(key => item[key] === null) || typeof item.mask_rationale !== "string" || !item.mask_rationale) throw new Error(`${label} mask provenance is incomplete`); verifyFile(root, item.mask_path, `${label}.mask_path`, item.mask_sha, true); }
}
...
function validate(options) {
  requireOptions(options, ["root"]); const root = rootInfo(options.root); return withLock(root.root, () => validateLocked(root));
}
function validateLocked(root) {
  const loaded = loadManifest(root); if (process.env.RELEASE_EVIDENCE_VALIDATE_LOCK_READY) { fs.writeFileSync(process.env.RELEASE_EVIDENCE_VALIDATE_LOCK_READY, "locked\n"); const release = process.env.RELEASE_EVIDENCE_VALIDATE_LOCK_RELEASE; for (let attempt = 0; release && !fs.existsSync(release) && attempt < 1000; attempt += 1) sleep(10); if (release && !fs.existsSync(release)) throw new Error("validate lock test seam timed out"); }
  const errors = []; if (!loaded.manifest.expected_inventory) errors.push("expected inventory is not locked"); let inventoryValue;
  try { if (loaded.manifest.expected_inventory) { const inventoryPath = verifyFile(root, loaded.manifest.expected_inventory.path, "expected inventory", loaded.manifest.expected_inventory.sha256).path; inventoryValue = readJSON(inventoryPath, "expected inventory"); if (JSON.stringify(inventoryValue) !== JSON.stringify(loaded.manifest.expected_inventory.value)) throw new Error("embedded inventory copy differs from locked hashed inventory"); validateInventory(inventoryValue, loaded.manifest.base_sha); } } catch (error) { errors.push(error.message); }
  ...
  if (errors.length) throw new Error(errors.join("\n")); loaded.manifest.validated_snapshot_sha256 = validationSnapshotHash(loaded.manifest); loaded.manifest.validated_at = new Date().toISOString(); atomicJSON(loaded.file, loaded.manifest, "manifest"); console.log(`validated ${loaded.manifest.reports.length} report(s), ${loaded.manifest.capture_manifests.length} capture manifest(s), ${loaded.manifest.canonical_artifacts.length} canonical artifact(s)`);
}
...
function artifactCreate(options) {
  ...
  withLock(root.root, () => { const fresh = loadManifest(root); if (fresh.manifest.canonical_artifacts.some(item => item.canonical && item.path !== output)) throw new Error("a canonical artifact already exists"); atomicWrite(sums, `${dmg.sha256}  ${path.basename(dmg.path)}\n`, "checksum"); artifact.sha256sums = { path: sums, sha256: sha256File(sums) }; atomicJSON(output, artifact, "artifact"); const entry = { path: output, sha256: sha256File(output), candidate_sha: artifact.candidate_sha, canonical: true }; fresh.manifest.canonical_artifacts = fresh.manifest.canonical_artifacts.filter(item => item.path !== output); fresh.manifest.canonical_artifacts.push(entry); invalidateValidation(fresh.manifest); atomicJSON(fresh.file, fresh.manifest, "manifest"); }); console.log(output);
}
