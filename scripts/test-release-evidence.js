...
try {
  ...
  {
    const root = path.join(scratch, "capture-schema"), report = baseReport(root, "reviewer"); fs.mkdirSync(root, { recursive: true }); report.expected_visual_states = []; report.screenshots = []; const semantic = path.join(root, "capture-semantic.json"), image = path.join(root, "capture.png"); text(semantic, "{}\n"); png(image); ok(run(["init", "--run-id", "capture-schema", "--root", root, "--base-sha", report.base_sha]).status === 0, "capture init"); installInventory(root, report, { workstreams: [{ workstream: report.workstream, base_sha: report.base_sha, roles: [{ role: report.role, visual_states: [], performance_cases: [], capture_manifests: [{ kind: "external", flow: "preflight" }], allow_zero_performance: true }] }] }); const capture = path.join(root, "capture.json"); json(capture, { kind: "external", flow: "preflight", status: "PASS", artifacts: [{ case_id: "window", iteration: 1, visual_state_id: "main", role: "actual", readiness: "ready", inspection: "PASS", path: image, sha256: hash(image), semantic_artifact: semantic, semantic_sha256: hash(semantic) }] }); const reportFile = path.join(root, "report.json"); json(reportFile, report); ok(run(["ingest", "--root", root, "--report", reportFile, "--capture-manifest", capture]).status === 0 && run(["validate", "--root", root]).status === 0, "deep external capture manifest validates"); text(image, "changed"); ok(run(["validate", "--root", root]).status !== 0, "changed capture artifact is rejected");
  }
  {
