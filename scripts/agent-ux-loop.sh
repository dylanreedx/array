#!/usr/bin/env bash
# agent-ux-loop.sh — Ralph-style driver for the 90-agent-ux program (forked from
# overnight-orchestration-loop.sh, which is retained untouched for the prior program). Forked from overnight-loop.sh; same observability + resilience, but each
...
PROMPT_FILE="${PROMPT_FILE:-scripts/agent-ux-prompt.md}"
QUEUE_FILE="${QUEUE_FILE:-docs/38-tickets/90-agent-ux/_QUEUE.md}"
