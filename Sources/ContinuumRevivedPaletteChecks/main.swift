...
expect(rows.map(\.displayName) == ["Shell", "Claude CLI Terminal", "New Agent…", "New Agent Without a Tile…", "Fan Out Selected Tickets…", "New Note", "New Browser", "Open File...", "Open File Tree...", "New Diff Review", "Fit Canvas to All", "Back to Previous View", "Go to Previous Tile", "Go to Previous Zone", "New Workspace…", "Create Zone…"], "palette appends note/browser/file/file-tree/diff/fit/workspace actions after profiles, then Create Zone")
...
expect(LaunchPaletteModel.filterRows(rows, query: "agent").map(\.displayName) == ["New Agent…", "New Agent Without a Tile…", "Fan Out Selected Tickets…"], "agent query matches the three agent actions in declared order (tile-bound first, then the P2D.6 fan-out), not raw CLI profiles")
expect(LaunchPaletteModel.filterRows(rows, query: "headless").map(\.displayName) == ["New Agent Without a Tile…"], "headless query matches only the tile-less agent action (P2A.6)")
...
expect(harnessRows.map(\.displayName) == ["New Agent…", "New Agent Without a Tile…", "Fan Out Selected Tickets…", "New Note", "New Browser", "Open File...", "Open File Tree...", "New Diff Review", "Fit Canvas to All", "Back to Previous View", "Go to Previous Tile", "Go to Previous Zone", "New Workspace…", "Run QA Reviewer Agent…", "Create Zone…"], "palette appends harness role actions after built-in actions, then Create Zone")
...
expect(jumpRows.map(\.displayName) == ["New Agent…", "New Agent Without a Tile…", "Fan Out Selected Tickets…", "New Note", "New Browser", "Open File...", "Open File Tree...", "New Diff Review", "Fit Canvas to All", "Back to Previous View", "Go to Previous Tile", "Go to Previous Zone", "New Workspace…", "Jump to API Server", "Jump to Notes", "Create Zone…"], "palette appends Jump-to-tile rows after the built-in actions, then Create Zone")
...
let builtIns7 = ["New Agent…", "New Agent Without a Tile…", "Fan Out Selected Tickets…", "New Note", "New Browser", "Open File...", "Open File Tree...", "New Diff Review", "Fit Canvas to All", "Back to Previous View", "Go to Previous Tile", "Go to Previous Zone", "New Workspace…"]
...
expect(projectRows.map(\.displayName) == ["New Agent…", "New Agent Without a Tile…", "Fan Out Selected Tickets…", "New Note", "New Browser", "Open File...", "Open File Tree...", "New Diff Review", "Fit Canvas to All", "Back to Previous View", "Go to Previous Tile", "Go to Previous Zone", "New Workspace…", "Create Zone…", "Add Work Project to Canvas"], "palette appends Create Zone then add-project rows")
...
expect(workspaceRows.map(\.displayName) == ["New Agent…", "New Agent Without a Tile…", "Fan Out Selected Tickets…", "New Note", "New Browser", "Open File...", "Open File Tree...", "New Diff Review", "Fit Canvas to All", "Back to Previous View", "Go to Previous Tile", "Go to Previous Zone", "New Workspace…", "Create Zone…", "Switch to Client Work Workspace", "Rename Client Work Workspace…", "Delete Client Work Workspace…"], "palette appends Create Zone then workspace rows")
...
// RETIRED BY INVERSION (.plans/10-command-center-absorbs-sidebar.md): the sidebar
// is not mounted any more, so searching for it must find NO command — offering one
// would be a control for a surface the user cannot see. Asserted as absent rather
// than deleted, so re-exposing the action fails here instead of shipping.
expect(LaunchPaletteModel.filterRows(workspaceRows, query: "workspace sidebar").isEmpty, "the unmounted sidebar's action must not be offered by the palette")
...
// Index [13] = Create Zone… (selectable), [14] = Switch to Empty Workspace (not
// selectable because empty). These moved up one when P2D.6 added `agent.fanOut`
// to the registry, and up one again when the workspace-sidebar toggle stopped
// being palette-visible (.plans/10-command-center-absorbs-sidebar.md) — the rows
// are positional, and the count is asserted above.
expect(emptyWorkspaceRows[13].isSelectable == true, "Create Zone action row at index 13 is selectable")
expect(emptyWorkspaceRows[14].isSelectable == false, "empty workspace switch rows are not selectable")
...
expect(missingRows[3].isSelectable, "New Agent Without a Tile action row is selectable")
expect(missingRows[4].isSelectable, "New Note action row is selectable")
expect(missingRows[5].isSelectable, "New Browser action row is selectable")
expect(missingRows[6].isSelectable, "Open File action row is selectable")
expect(missingRows[7].isSelectable, "Open File Tree action row is selectable")
expect(missingRows[8].isSelectable, "New Diff Review action row is selectable")
expect(missingRows[9].isSelectable, "Fit Canvas to All action row is selectable")
expect(missingRows[10].isSelectable, "Back to Previous View action row is selectable")
expect(missingRows[11].isSelectable, "Go to Previous Tile action row is selectable")
expect(missingRows[12].isSelectable, "Go to Previous Zone action row is selectable")
// Index 13 was the sidebar toggle; it is no longer offered, so New Workspace moved
// up one (.plans/10-command-center-absorbs-sidebar.md).
