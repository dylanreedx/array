...
public enum AgentModelConfig {
    ...
    /// harness can be Claude Code while the caller is Pi, and `anthropic/opus` is
    /// exactly the kind of pattern `pi --model` fuzzy-matches across a provider's
    /// models — the ambiguity P0.10 exists to prevent.
