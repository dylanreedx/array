...
/// A coordinator-owned review surface for the original Orbit and its five
/// variations. This is intentionally separate from the four-candidate study:
/// no candidate is selected or routed into the production managed-agent tile.
@MainActor
final class OrbitVariationsGalleryView: NSView {
    enum Mode: Equatable {
        case live
        case snapshot
    }
    ...
    enum Candidate: String, CaseIterable {
        case orbit = "Orbiting Triad"
        case chromaticRelay = "Chromatic Relay"
        case tiltedPrism = "Tilted Prism"
        case counterCurrent = "Counter-Current"
        case solarHandoff = "Solar Handoff"
        case angularSlingshot = "Angular Slingshot"

        var identifier: String {
            switch self {
            case .orbit: return "orbit"
            case .chromaticRelay: return "chromatic-relay"
            case .tiltedPrism: return "tilted-prism"
            case .counterCurrent: return "counter-current"
            case .solarHandoff: return "solar-handoff"
            case .angularSlingshot: return "angular-slingshot"
            }
        }

        var direction: String {
            switch self {
            case .orbit: return "Circular triad"
            case .chromaticRelay: return "Colour relay"
            case .tiltedPrism: return "Tilted plane"
            case .counterCurrent: return "Counter-rotating"
            case .solarHandoff: return "Lead handoff"
            case .angularSlingshot: return "Kite slingshot"
            }
        }
        ...
        func makeIndicator(reducedMotion: Bool) -> NSView & AgentThinkingIndicatorAnimating {
            switch self {
            case .orbit:
                return OrbitingTriadThinkingIndicatorView(reducedMotion: reducedMotion)
            case .chromaticRelay:
                return ChromaticRelayOrbitThinkingIndicatorView(reducedMotion: reducedMotion)
            case .tiltedPrism:
                return TiltedPrismOrbitThinkingIndicatorView(reducedMotion: reducedMotion)
            case .counterCurrent:
                return CounterCurrentOrbitThinkingIndicatorView(reducedMotion: reducedMotion)
            case .solarHandoff:
                return SolarHandoffOrbitThinkingIndicatorView(reducedMotion: reducedMotion)
            case .angularSlingshot:
                return AngularSlingshotOrbitThinkingIndicatorView(reducedMotion: reducedMotion)
            }
        }
    }
    ...
    static let preferredSize = NSSize(width: 940, height: 600)
    private let titleLabel = NSTextField(labelWithString: "Orbit Variations — Motion Study")
    private let subtitleLabel = NSTextField(labelWithString: "Original Orbit plus five focused variations at true 18×18 scale. Compare the same compact managed-agent status row in Normal and Reduced Motion; this is review-only.")
    private let grid = NSView()
    private let mode: Mode
    private var cards: [OrbitVariationCardView] = []
    ...
    init(frame frameRect: NSRect, mode: Mode) {
        ...
        identifier = NSUserInterfaceItemIdentifier("managedAgent.orbitVariations.gallery")

        titleLabel.identifier = NSUserInterfaceItemIdentifier("orbitVariations.title")
        titleLabel.font = .systemFont(ofSize: 17, weight: .semibold)
        titleLabel.lineBreakMode = .byTruncatingTail
        titleLabel.setContentCompressionResistancePriority(.required, for: .vertical)

        subtitleLabel.identifier = NSUserInterfaceItemIdentifier("orbitVariations.subtitle")
        subtitleLabel.font = .systemFont(ofSize: 11, weight: .regular)
        subtitleLabel.lineBreakMode = .byWordWrapping
        subtitleLabel.maximumNumberOfLines = 2

        grid.translatesAutoresizingMaskIntoConstraints = false
        grid.identifier = NSUserInterfaceItemIdentifier("orbitVariations.grid")
        ...
        setAccessibilityLabel("Orbit Variations motion study")
    }
    ...
    private func buildGrid() {
        for reducedMotion in [false, true] {
            for candidate in Candidate.allCases {
                let card = OrbitVariationCardView(candidate: candidate, reducedMotion: reducedMotion, mode: mode)
...
@MainActor
private final class OrbitVariationCardView: NSView {
    let candidate: OrbitVariationsGalleryView.Candidate
    let reducedMotion: Bool
    let indicator: NSView & AgentThinkingIndicatorAnimating
    let nameLabel: NSTextField
    let directionLabel: NSTextField
    ...
    init(candidate: OrbitVariationsGalleryView.Candidate, reducedMotion: Bool, mode: OrbitVariationsGalleryView.Mode) {
        ...
        self.whereLabel = NSTextField(labelWithString: "feature/orbit-variations")
        ...
        identifier = NSUserInterfaceItemIdentifier("orbitVariations.card.\(candidate.identifier).\(reducedMotion ? "reduced" : "normal")")

        nameLabel.identifier = NSUserInterfaceItemIdentifier("orbitVariations.name.\(candidate.identifier).\(reducedMotion)")
        nameLabel.font = .systemFont(ofSize: 11, weight: .semibold)
        nameLabel.lineBreakMode = .byWordWrapping
        nameLabel.maximumNumberOfLines = 2
        nameLabel.setContentCompressionResistancePriority(.required, for: .vertical)
        directionLabel.identifier = NSUserInterfaceItemIdentifier("orbitVariations.direction.\(candidate.identifier).\(reducedMotion)")
        ...
        motionLabel.identifier = NSUserInterfaceItemIdentifier("orbitVariations.motion.\(candidate.identifier).\(reducedMotion)")
        ...
        statusLabel.identifier = NSUserInterfaceItemIdentifier("orbitVariations.status.\(candidate.identifier).\(reducedMotion)")
        ...
        indicator.identifier = NSUserInterfaceItemIdentifier("orbitVariations.indicator.\(candidate.identifier).\(reducedMotion)")
        ...
        setAccessibilityElement(true)
        setAccessibilityRole(.group)
        setAccessibilityLabel("\(candidate.rawValue), \(candidate.direction), \(reducedMotion ? "Reduced Motion" : "Normal"), Claude, feature/orbit-variations, Thinking")
        applyAppearance()
    }
