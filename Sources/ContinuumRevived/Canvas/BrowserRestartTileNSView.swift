...
/// Placeholder tile view for browser tiles whose runtime failed to start.
/// Mirrors `TerminalRestartTileNSView` with browser-tinted copy and chrome.
/// Restart button calls back through the supplied closure; nil disables the
/// affordance for non-recoverable error states.
@MainActor
final class BrowserRestartTileNSView: TileNSView {
    ...
    init(tile: Tile, statusText: String, onRestart: (() -> Void)?) {
        ...
        body.layer?.backgroundColor = NSColor(white: 0.93, alpha: 1.0).cgColor
