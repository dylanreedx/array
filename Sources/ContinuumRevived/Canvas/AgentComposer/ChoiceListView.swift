...
final class ChoiceListView: NSView, TokenThemed {
    ...
    override var intrinsicContentSize: NSSize {
        ...
        return NSSize(
            width: max(Self.minimumWidth, textWidth + Self.horizontalPadding * 2 + 26),
    ...
    override func layout() {
        super.layout()
        var y = bounds.height - Self.verticalPadding - Self.rowHeight
        for row in rows {
            row.frame = NSRect(
    ...
    private func rebuildRows() {
        ...
        rows = items.map { item in
            let row = ChoiceRowView(item: item)
...
private final class ChoiceRowView: NSControl, TokenThemed {
    ...
    var qaCheckHidden: Bool { checkView.isHidden }
    ...
    override func layout() {
        super.layout()
        checkView.frame = NSRect(x: 8, y: floor((bounds.height - 14) / 2), width: 14, height: 14)
    ...
    func applyTokens() {
        ...
        checkView.contentTintColor = AgentLineRole.focusRing.color.nsColor(for: theme)
