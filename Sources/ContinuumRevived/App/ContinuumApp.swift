...
enum ContinuumApp {
    ...
    static func main() {
        ...
        if CommandLine.arguments.contains("--terminal-scale-diagnose-check") {
            let application = NSApplication.shared
            application.setActivationPolicy(.accessory)
            do {
                let artifact = try GhosttyTerminalRuntime.runTerminalScaleDiagnoseSelfCheck()
                print("ContinuumRevivedTerminalScaleDiagnoseChecks passed: \(artifact.path)")
                Foundation.exit(0)
            } catch {
                fputs("FAIL: \(error)\n", stderr)
                Foundation.exit(1)
