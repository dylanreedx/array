...
| C-20260703-019 | `74-agent-message-bus-seam.md` | open | `CONTRACT-CONFLICT`, `REALPATH-MISSING` | run `wf_e103dbc2-d70` (3 rounds, Claude clear, Codex rejected) | The Backend real-path check (instantiate real `ZoneRuntimeController`, call `controller.agentBus.post`) and the Done-when grep gate (zero `agentBus.post`/`agentBus.subscribe` hits in `Sources/ContinuumRevived` beyond the property decl) are mutually exclusive: `ZoneRuntimeController` is in the non-importable app target, so the only real-path site is a flag-wired self-check inside `Sources/ContinuumRevived`, whose `post` call trips the grep. **Required ruling:** either exempt the flag-wired self-check call site from the grep gate (same shape as C-20260702-010 for ticket 14), or make `ZoneRuntimeController` library-importable so the check moves to a `*Checks` target. Pure Core half (protocol/struct/enum/null impl + `ingestBusMessage` + 3 logic checks) was Claude-clear. Conceptual dep on the unbuilt supervised observer (ticket 40) is non-structural and did NOT block the code. Rejected attempt stashed (`stash@{0}`). | Dylan/overwatch | - |
