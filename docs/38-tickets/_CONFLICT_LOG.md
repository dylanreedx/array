...
| C-20260701-007 | `07-convergence-fuzz-red-green.md` | amended | `DEP-BLOCKED`, `DETERMINISM-GAP`, `REVIEW-REJECTED` | dry-run agents; run `20260702T230208` iter-2 (3 rounds, both rejected) | Dep on 06 (landed 880cec0). **DETERMINISM RULING (2026-07-02):** seed-derived fixed UUIDs/timestamps only. **RETRY RULINGS (2026-07-03):** (1) **ARCH — Dylan's ruling:** Apple Silicon (arm64) is the only supported arch; the seed-1 canonical byte-pin may stay but must be documented arm64-only; the x86_64 matrix arm is DROPPED. (2) **ORACLE (required):** every seed must assert final converged bytes `== canonicalEncode(materialize(all generated ops))` — convergence alone is insufficient. (3) **DROP ADVERSARY (required):** the delivery adversary must be able to permanently DROP unselected messages, not just delay/reorder/dup. (4) **COMPACTION (overwatch sign-off, flagged for Dylan):** `safeMark` compacting only through fully-acked points ACCEPTED as correct-by-design per the ticket's own "provably acked" rule; watermark-race fuzzing deferred to ticket 56. Prior attempt stashed. RETRYABLE NOW. | Dylan/overwatch | - |
| C-20260701-008 | `09-taint-scan-i5.md` | amended | `DEP-BLOCKED`, `CONTRACT-CONFLICT`, `REVIEW-REJECTED` | dry-run agents; run `20260702T230208` iter-3 (Claude clear, Codex rejected) | 08 landed (368cf7e). **TAINT RULING (2026-07-02):** small-integer geometry is NOT taint; pid/handle/pane-id-shaped values + host paths ARE. **RETRY RULINGS (2026-07-03):** (1) **SCALAR LEAVES ONLY:** the geometry exemption applies to scalar numeric leaves (`frame.x/y/width/height` as scalars) and must NEVER propagate through arrays or subtrees — `SyncPayloadTaintScanner.swift:121-124,160-162` let a pid hide inside `frame.x[12345]`; add probes covering exactly that bypass. (2) **OVERFLOW FIXTURE (required):** a fixture with a genuine `UInt64 > Int.max` value must exercise the Done-when #3 overflow guard (current fixtures max at 5M and use `Int`). RETRYABLE NOW. | Dylan/overwatch | - |
...
| C-20260703-018 | `70-approvals-needsattention.md` | open | `DEP-BLOCKED` | queue readiness check | "Depends on" (line 249) names the **ACP driver (ticket 69)** — not done, not in the autonomous queue, no ledger row. Independently, the ticket's mandatory Backend real-path check is explicitly **non-bypassable** ("This check must not call `deriveAgentStatus` directly. It drives the full event path — adapter event → store update → **observer cycle** → store read") and needs a `SessionObserver` tick, but `SessionObserver` (ticket **40**, Supervised) does not exist on disk (grep-confirmed — only a comment mention; same block as C-20260703-015). `respondToApproval`'s adapter-forward path uses `controller.managedAdapters[…]`, and no `managedAdapters` seam exists (ticket 66/69 territory; 66 skipped). The pure logic checks alone (`AgentApprovalRequest` semantics + `deriveAgentStatus`) WOULD compile — ticket 67's (2979905) `AgentRuntimeEvent.requestOpened/requestResolved` cases are present in `AgentStatusEngine.swift` — but "Done when" items 5/6/8 and the whole real-path section require the observer/adapter production path; shipping only the pure half + stubbing the mandatory real-path check is the fake-green the runbook forbids. Do not attempt until 40 (observer) + 69 (ACP driver) are built by hand, then re-attempt (auth-scope ruling C-20260702-012 still applies). Marked skipped in `_PROGRESS.md`. | Dylan/overwatch | - |
| C-20260703-019 | `74-agent-message-bus-seam.md` | amended | `CONTRACT-CONFLICT`, `REALPATH-MISSING` | run `wf_e103dbc2-d70` (3 rounds, Claude clear, Codex rejected) | The Backend real-path check (instantiate real `ZoneRuntimeController`, call `controller.agentBus.post`) and the Done-when grep gate (zero `agentBus.post`/`agentBus.subscribe` hits in `Sources/ContinuumRevived` beyond the property decl) are mutually exclusive under the app-target architecture. **RULING (overwatch 2026-07-03, precedent C-20260702-010/ticket 14):** the flag-wired self-check call site inside `Sources/ContinuumRevived` is EXEMPT from the grep gate — the gate becomes "zero hits other than the property declaration AND the flag-wired self-check method"; the self-check must be gated behind its check flag so no production path posts. Pure Core half was Claude-clear; prior attempt stashed (`stash@{0}`) and may be salvaged. RETRYABLE NOW. | Dylan/overwatch | - |
| C-20260703-020 | `58-activity-projection-transport.md` | amended | `REVIEW-REJECTED`, `DETERMINISM-GAP` | run `20260702T230208` iter-9 (3 rounds, both rejected) | Two findings, both diagnosed to the line. **RULINGS (2026-07-03):** (1) **SYNC REGISTRATION (required, the real production bug):** demux subscriptions MUST be registered synchronously BEFORE any send, in both `ActivityProjectionReceiver.connect()` (sends `.activitySubscribe` before its receiveLoop Task registers — `ActivityProjectionReceiver.swift:73`, `SyncMessageDemux.swift:52` drain-time broadcast) and symmetric `ActivityProjectionSender.start()`; add a Checks case with a PRE-EXISTING spatial subscriber on the SAME shared demux (the race the fresh/private-demux checks never gate). (2) **DETERMINISTIC HARNESS (required):** the loopback transport's count-based delay strands trailing replay/cold-reconnect messages (~25% red) — the harness must drain/flush pending until convergence-or-timeout, deterministic over many runs, and prove convergence happens via the REAL replay/cold-reconnect path, not a lucky extra send. Prior attempt stashed. RETRYABLE NOW. | Dylan/overwatch | - |
...
| C-20260703-021 | `09-taint-scan-i5.md` | open | `HARNESS-FAIL`, `REVIEW-REJECTED` | run `wf_8c2deae7-852` (3 rounds) | **False-positive, not an impl defect.** The ticket-09 implementation is substantively dual-cleared: Claude review "merge-ready"; Codex cleared every *substantive* concern across the 3 rounds — the array-crossing geometry-exemption bypass (`{"frame":[{"x":12345,…}]}`, where `stripArrayIndex` re-verified an array-indexed dict as geometry) fixed in round 2; the stale `outcome=.stub` I5 InvariantManifest reconciled in round 3; scalar-leaf-only exemption + a genuine `UInt64 > Int.max` overflow fixture present per C-008. Codex's **sole** remaining block is that `git status` shows an untracked `scripts/codex-primary-loop.sh` — a human-dropped daytime Codex-primary-driver reconfiguration artifact, entirely outside ticket scope; the loop did not create it and must not move it. **RECOVERY:** set the stray script aside (human reconfiguration file), then either commit the staged `SyncPayloadTaintScanner.swift` + `main.swift` taint block directly, or re-drive the workflow on the now-clean tree (Codex's only objection evaporates). Collateral: the workflow reverted the legitimate uncommitted 07 done-ledger row when Codex round-1 flagged it as unrelated — restored in this iteration. Loop `STOP`'d (STOP file 07:32) mid-reconfiguration; not re-driven autonomously. | Dylan/overwatch | - |
