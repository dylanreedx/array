...
> **Update — 2026-06-30 spike results.** Three grounded spikes resolved the major open
> forks; verdicts are folded into the decisions below, with full analysis in
> [`docs/2026-06-30-orchestration-spikes/`](2026-06-30-orchestration-spikes/):
> - **Sync (`SYNC-MODEL.md`):** **deterministic op-log**, not an off-the-shelf CRDT —
>   *contingent* on low single-user contention and the I4 fuzz going green early (write it
>   RED→GREEN first). Fallback if either breaks or iOS becomes a true concurrent editor:
>   **Loro 1.x** (MIT, native MovableList; ~54–124 MB FFI zip — measure the real linked
>   delta before reversing). Op-log makes I5 a **type-level** guarantee but re-models
>   membership → tile-level LWW register, z-order → fractional index, delete → tombstone.
> - **Topology (`TOPOLOGY.md`):** ambient/group-zone tiles → **per-workspace session
>   `continuum-ws-<id>`**, behind a per-tile fallback for phase 1 (per-zone rejected: no
>   production membership write-path — `setTiles` is clear-only, `ContinuumApp.swift:6334`).
>   Project-release → **DETACH, never kill** (projects are shared across workspaces).
>   Make-or-break seam: capture + persist `tmuxWindowTarget` (`%pane_id`) **at spawn** with
>   a dead-target→`new-window` fallback — there is **no production tmux-query code** today.
> - **Readers (`AGENT-READERS.md`):** Claude link is exact (`sessions/<pid>.json` →
>   `sessionId`). **Pi** runs are **project-local** `<projectRoot>/.pi/agent-runs/<runId>`
>   (not `~/.pi`), `runId` = run-dir basename = an exact key; reuse the existing
>   `RunArtifactsWatcher`. **Codex has no pid/tty link** → recency-by-mtime + cwd
>   (collision risk on shared cwd). Claude `needsAttention` is **not file-derivable** in
>   `bypassPermissions` mode → **hook-driven only** until a non-bypass fixture proves it;
>   under-claim rather than fabricate.
>
> **Cross-cutting insight:** topology and sync independently converged on the same weak
> spot — **group-zone membership**. Both want it re-modeled as a **property of the tile**
> (an LWW register), not the separate `groupZoneTiles` list (whose write-path barely
> exists). That re-modeling is now a shared centerpiece of Decisions A + E.
...
- `TileSpawner.spawnTerminal` `:108`, `tmuxWrappedProfileIfAvailable` `:221`,
  ...
  `flushTerminalSessionSnapshot` `:371`. A `send-keys "cd <path>"` exists at `:3473` but
  **inside a self-check, not a production path** — there is effectively **no production
  tmux-query/IPC code** today, and `listSessions()` reads on-disk descriptors, **not**
  `tmux ls`. (TOPOLOGY spike, 2026-06-30.)
...
- **The make-or-break seam:** add a durable `tmuxWindowTarget` (`%pane_id`) to
  `TerminalSessionDescriptor`, **captured and persisted synchronously at spawn**, with a
  dead-target → `new-window` fallback. Once N tiles share one session, nothing else
  identifies *which* window is a given tile (today's implicit `new-session -A` rebind
  collapses). Done lazily, **I1 and I8 break** and every restart silently re-creates
  windows. (TOPOLOGY spike.)
...
| Last tile in project closed | n/a | session dies when its window count hits 0 |
| App quit / restart | session survives, reattach | unchanged — session survives, windows intact |
| Project released (`ZoneRuntimeRegistry` → 0) | n/a | **DETACH — session stays alive.** Projects are shared across workspaces; killing here reaps live agents on a workspace switch. *(Resolved: TOPOLOGY 2026-06-30.)* |

**Resolved in A (TOPOLOGY 2026-06-30):**
- **Ambient/group-zone tiles** (`ZonePlacement.projectId == nil`) → a **per-workspace
  session `continuum-ws-<workspaceId>`**, shipped behind a **per-tile fallback** for
  phase 1 (the existing, tested kill-on-delete path). Per-zone rejected: it depends on a
  `groupZoneTiles` membership write-path that **does not exist in production**
  (`setTiles` is clear-only, `ContinuumApp.swift:6334`).
- **Migration:** bind via the new `tmuxWindowTarget`; live `continuum-<tileId>` sessions
  don't fold into windows, so **start fresh project sessions on upgrade** (agents restart
  once) with a one-time note.
- **cwd inheritance:** a new tile takes the focused tile's `pane_current_path`, else
  project root.
...
Because every tile is now a window in a session, tmux tells us cheaply **what a tile
is and whether it's alive** — but agents already persist their *own* authoritative
session state on disk, so for **what an agent is doing** we read that; we do not scrape
the terminal. (Stores below verified on this machine, 2026-06-30.)

**Detection + liveness — tmux, occasional:**
- `tmux display -p -t <win> '#{pane_current_command}'` → foreground process
  (`zsh`, `claude`, `codex`, `pi`): the kind classifier.
- pane **pid** alive + `pane_current_path` (OSC 7, already captured): liveness + the
  join key into the agent's own store.

**Status — each agent's own store, authoritative:**

| Agent | Store (verified) | Exact link to a tile | Status signal |
|---|---|---|---|
| Claude Code | `~/.claude/projects/<encode(cwd)>/<sessionId>.jsonl` (events) | **pane pid → `~/.claude/sessions/<pid>.json` → `sessionId`** | last event type, `permission-mode`, `ai-title`, mtime |
| Codex | `~/.codex/sessions/**/rollout-*.jsonl` (`session_index.jsonl` is **stale/partial**, not a reliable index) | **no pid/tty link** → recency-by-mtime + `cwd` match on line-1 `session_meta` (collision risk on shared cwd) | rollout events, mtime |
| Pi | **project-local** `<projectRoot>/.pi/agent-runs/<runId>/` (Continuum-spawned); `~/.pi/overnight-runs/<project>/{latest,run-*}/` (overnight harness) | **`runId`** = run-dir basename = exact key (already on `AgentDescriptor`) | **explicit `status.json`** (+ `run.json`, `events.jsonl`); reuse existing `RunArtifactsWatcher` |

`encode(cwd)` = path with `/` and `.` → `-` (this repo →
`-Users-dylan-Documents-personal-continuum-revived`). Two facts make this clean:
**Claude's link is exact** — `~/.claude/sessions/<pid>.json` keys by process id (e.g.
`38649.json`), so a pane maps straight to its session with no cwd-collision guessing —
and **Pi exposes an explicit `status.json`** per run. Terminal output is never the
source of truth.

So the integration is **not** a polymorphic scraper; it is a thin per-agent
**`AgentStateReader`**:

```
AgentStateReader (protocol)
  ├─ kind: AgentKind                       // claude | codex | pi
  ├─ detect(processName) -> Bool           // pane_current_command
  ├─ locate(pid, cwd, runId?) -> URL?      // the agent's own session/run store
  └─ read(URL) -> AgentSnapshot            // {status, title?, mode?, asOf}
```

- **Exact linkage when we spawn the agent:** capture the `runId` (Pi) or read
  `sessions/<pid>.json` (Claude); `AgentDescriptor.runId` already exists for this.
- **No output-scraping.** `capture-pane` is a last-resort fallback only; an agent with
  no reader shows as `shell`/`unknown` (running vs idle from the process signal alone),
  never a guessed status.
- **Push beats poll.** These stores move on every agent action, so **watch the file**
  (FSEvents) for push-latency without polling; Claude also supports hooks
  (`~/.claude/settings.json` already carries a `hooks` block) for an explicit
  Stop/Notification breadcrumb. tmux stays out of the status hot path.
- **Privacy is a hard line (I5).** Readers extract **metadata only** — event *type*,
  mode, title, timestamps — **never message bodies**; transcript content must never
  cross the sync boundary.
- **Remote:** the store lives wherever the agent runs; a remote agent's files are read
  over the same host channel as Decision D — same reader, different transport.

A per-project **`SessionObserver`** (in `ZoneRuntimeController`) runs detection, drives
the readers, and writes each result into the existing **`AgentDescriptor.status`** —
the field the whole UI already consumes. An unknown agent still appears (as
`shell`/`unknown`); it simply lacks deep status until a reader exists. **Extensible,
...
**Resolved/open in C (AGENT-READERS 2026-06-30):** Codex has **no pid/tty link** — use
recency-by-mtime + cwd (collision risk when tiles share a cwd; *open:* disambiguation).
Claude **`needsAttention` is not file-derivable in `bypassPermissions` mode** → spec it
**hook-driven only** until a non-bypass golden fixture proves a file signal; under-claim
to `working`/`idle` rather than fabricate. Still open: store formats are
**version-unstable** (golden fixtures I6 + maintenance, accepted); read-only file-watch
vs. **installing** a hook into `~/.claude/settings.json` (needs consent); FSEvents watch
budget + debounce at many tiles (reuse `RunArtifactsWatcher`'s pattern).
...
**Resolved/open in E:** the sync **model** = deterministic op-log (SYNC-MODEL
2026-06-30), which **re-models the spatial state** — membership becomes a tile-level LWW
register (not `groupZoneTiles`), z-order a fractional index, delete a tombstone. Still
open: sync **transport** (CloudKit-first, relay as upgrade); iOS client scope
(observer-only first); identity/account model; and **measuring the real linked app-size
delta** of the Loro fallback before any reversal.
...
- **Agent detection / status (I6):** **golden fixtures** — recorded real
  `pane_current_command` + the agents' own session stores (`~/.claude/projects/*.jsonl`,
  `~/.codex/sessions`, `~/.pi/.../status.json`) for shell/claude/codex/pi, replayed
  through the readers. Decouples reader logic from live agents. The observer carries
  **counters/budgets** (events/sec, status-changes/min) so it cannot thrash.
- **View binding (I2):** a real-path check proving two tiles render two *different*
  windows of one session, not a mirror.

### The sync-model fork — RESOLVED (SYNC-MODEL 2026-06-30)
The single decision that determines whether sync is *provable* or *perpetually flaky*:

> **Decision:** a **hand-rolled deterministic op-log** (pure-Swift, Lamport + replicaId
> total order), *contingent* on low single-user contention and the I4 fuzz going green
> early. Fallback: **Loro 1.x** (MIT, native MovableList) if contention rises or iOS
> becomes a concurrent editor. The op-log makes **I5 type-level** (runtime handles aren't
> representable in the `Op` enum) but requires re-modeling membership → tile-level LWW
> register, z-order → fractional index, delete → tombstone.
...
2. **Close tile = kill-window; session dies at 0 windows. Project release = DETACH,
   never kill** (projects are shared across workspaces). *(Refines `docs/34` #1;
   resolved by TOPOLOGY 2026-06-30.)*
...
9. **Sync model = deterministic op-log** (pure-Swift, Lamport + replicaId), *contingent*
   on low contention + the I4 fuzz going green early; **Loro 1.x** is the documented
   fallback. Re-models membership / z-order / delete (see Decision E). *(Resolved:
   SYNC-MODEL 2026-06-30.)*
10. **Managed-agent tier (FORK — needs Dylan):** optionally add a **headless managed-agent
    tile kind** driven via adapters (ACP / `codex app-server` / Claude SDK) *alongside*
    shell tiles + readers — unlocking authoritative status, approvals→`needsAttention`,
    and push. Gated on the **drive-vs-observe** decision (+ Node-sidecar-vs-Swift). See
    § "t3code prior-art & refinements" and `docs/2026-06-30-t3code-steal/`.
...
1. **Project = session, tile = window (A)** + migration + **`tmuxWindowTarget` capture
   at spawn** (the make-or-break seam). Kills the re-cd pain.
2. **De-mirror via grouped sessions (B).** Now that tiles are windows.
3. **SessionObserver + AgentStateReader registry (C, base + generic).** Auto-populate
   `AgentStatus`; replace the `:3176` mock; reuse `RunArtifactsWatcher`'s watch pattern.
4. **Render the activity tree / sidebar (C + `docs/35`).**
5. **Rich readers (C): Pi (`status.json`), Claude Code (pid→session), Codex.**
...
just core math. The sync model is **decided** (op-log, contingent — SYNC-MODEL); the
tripwire is to write the **I4 convergence fuzz RED→GREEN first**, before the real
`SyncTransport`, and switch to the Loro fallback if it can't go green cheaply.
...
- **`tmuxWindowTarget` discipline (A):** if the `%pane_id` isn't captured + flushed
  before teardown, I1/I8 break and restarts re-create windows. The make-or-break seam.
- **Codex same-cwd collision (C):** with no pid link, two Codex tiles in one cwd are
  file-indistinguishable; recency-by-mtime disambiguates but isn't bulletproof.
- **Fabricated status (C):** never emit `needsAttention`/`done` we can't back — Claude
  attention is hook-only until proven; prefer honest `unknown`.
- **ssh reliability (D):** dropped links must degrade gracefully (stale, not wrong).
- **Op-log merge ownership (E):** we own the merge; mitigations are the I4 fuzz as a hard
  gate (RED→GREEN first) and the Loro fallback if it won't converge cheaply.

---

## t3code prior-art & refinements (2026-06-30)

Six agents mined `pingdotgg/t3code` (the architectural **inverse** — server owns all
runtime; thin clients over one authed WS-RPC). Full analysis + code snippets in
[`docs/2026-06-30-t3code-steal/`](2026-06-30-t3code-steal/) (README + 6 area docs);
tickets in its `TICKETS.md`. Refinements to the decisions above:

- **D (remote) → a reach-path *menu*, not just ssh-wrap.** `RemoteReach = localhost |
  sshForward | tailscale | tunnel`, modeled as "how to reach + a revival recipe," all
  converging on one attach target. Continuum's twist vs t3: we forward a **tmux attach**,
  not a WebSocket — so `sshForward` = `ssh -t 'tmux attach -t <session>'` with **no
  `-L`/`-N`** (the attach *is* the remote command; port-forwarding returns only under
  tmux control-mode or a host daemon). Auth on **every** path incl. loopback via a seeded
  bootstrap grant (never `if (localhost) skip`) is what lets local/remote/iOS share one
  code path.
- **E (sync) → split sync from observation, enforced by a *type*.** Spatial = op-log
  (bidirectional). Activity tree = a **one-way projection** (host→observers),
  snapshot-then-tail. Write-authority is a type (client vs internal commands; only runtime
  ingestion writes agent state) → **I5 becomes type-level** three ways (scopes keep
  capability off the device; command-type keeps forged agent-state out; event-body
  exclusion keeps transcripts off the wire). And **two stores**: a private, host-local
  **`ManagedAgentSessionRecord`** (PK `tileId`; opaque `resumeCursor` + `runtimePayload`
  = the natural **home for the `tmuxWindowTarget %pane_id`**) that never syncs, vs. the
  derived `AgentStatus` that projects. Plus **lazy-resume-on-focus + idle reaper** (reap =
  **detach**, per TOPOLOGY).
- **C (agents) → a managed-agent tier, additive above the readers.** t3 drives agents
  **headlessly** via structured protocols (Claude SDK / `codex app-server` JSON-RPC /
  **ACP** for Cursor+Grok / OpenCode HTTP) behind one `ProviderAdapterShape` + a canonical
  event union + a **pure status fn** (satisfies I6 by construction). For Continuum this is
  a **new tile kind** (structured transcript + approvals), *not* "claude in a terminal" —
  shell/terminal tiles + the dotfile readers **stay**. **Approvals are the authoritative
  `needsAttention`** (a pending approval, checked above "running") — which **closes the
  AGENT-READERS gap, for managed agents only** (two regimes: managed = approvals, shell =
  hook/file). **ACP is the highest-ROI integration** (one client → Cursor/Grok/Gemini/Zed).
  Feeds the same iOS **APNS "agent needs you"** push.
- **Resilience.** Port t3's **`ConnectionSupervisor`** (`connected` only after socket-open
  **and** an initial config RPC; backoff `[1,2,4,8,16]s`; offline snapshot cache;
  `switchMap`-over-session durable subscriptions) for the iOS observer + remote-attach
  control. And adopt "**reattach by stable id + replay persisted scrollback**" as the
  *checkable acceptance contract* for Decision A's tmux binding (turns I1/I8 from prose
  into a test; on-screen replay is the deferred no-op at `TileSpawner.swift:354-355`).

**The central fork (needs Dylan): DRIVE vs OBSERVE.** Driving agents (managed tier)
unlocks authoritative status + approvals + push; observing keeps us on readers + the
Claude `Notification` hook. Sub-fork if drive: **Node sidecar** (reuse t3's TS drivers
verbatim) vs **pure-Swift** protocol clients. Gates tickets T3C-02…04, T3E-03; Group A
(reach paths, sync/observation split, two-stores, status fn, pairing model) ships
regardless.

**Does NOT transfer:** don't event-source or centralize the *spatial* layer (op-log +
offline-first win); no 60-method RPC surface (our transport is narrow); t3 streams pty
*bytes* to a web renderer — Continuum renders ghostty natively attached to tmux, so the
byte-stream + server-owned pty table apply only to the iOS observer, not the local path.
...
- **`docs/2026-06-30-orchestration-spikes/`** — the three grounded spikes (SYNC-MODEL,
  TOPOLOGY, AGENT-READERS) that resolved the major forks; see the 2026-06-30 Update at
  the top. Detailed backing for Decisions A, C, E.
- **`docs/2026-06-30-t3code-steal/`** — 6 code-grounded steal-docs + synthesis + `TICKETS.md`
  mining `pingdotgg/t3code` for the remote/transport/managed-agent patterns; see § "t3code
  prior-art & refinements." Backing for Decisions C, D, E.

---

## Open questions (master list, for iteration)

**Resolved 2026-06-30:** ~~ambient/group-zone session ownership~~ → per-workspace
session (TOPOLOGY); ~~project-release kill vs detach~~ → detach (TOPOLOGY); ~~sync model
CRDT vs op-log~~ → op-log, contingent (SYNC-MODEL); ~~Codex exact linkage~~ → none,
recency+cwd (AGENT-READERS).

Still open:
- Codex same-cwd collision disambiguation.
- Claude `needsAttention` file signal in non-bypass mode (needs a golden fixture).
- cwd-inheritance override policy for new tiles.
- Observer file-watch budget + debounce at scale; hook-install consent.
- `agentKind` enum/registry vs. free string.
- Remote: ssh-wrap vs. Continuum host daemon on the VPS.
- Sync **transport** (CloudKit vs relay) + iOS client scope (observe-only vs. control).
- Loro fallback: real linked app-size delta (measure before any A↔B reversal).
- Identity/account model once multi-device exists.
- **DRIVE vs OBSERVE agents** (managed adapters vs. readers) — the central fork; gates the
  managed-agent tier (t3code steal).
- If drive: **Node sidecar** (reuse t3's TS drivers) **vs. pure-Swift** protocol clients.
